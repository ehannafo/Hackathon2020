...
int length;
void play(string& answer, vector<string>& grid, int moves[9], char& player, char& player1, char& player2);
...
int main () {
    ...
    while (playAgain == "y") { //runs if player wants to play (again), runs main game
        ...
        int playNineTimes = 0;
        cout << "Choose a box, buddy!" << endl;
        while (winningConditions(answer, grid, moves) == false && playNineTimes < 9) {
            play(answer, grid, moves, player, player1, player2);
            playNineTimes ++;
        }
...
void play(string& answer, vector<string>& grid, int moves[9], char& player, char& player1, char& player2) {
    ...
    while (moves[boxNum - 1] != 0) {
        cout << "Sorry bud, that one's taken" << endl;
